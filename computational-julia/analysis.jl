...
## PAPER-STYLE (_mga) FIGURES
#
# The `_mga` variants are the ones the paper actually \input{}s. They differ
...
"Shared axis styling used by both `_mga` figures (one option per line, unindented)."
const MGA_AXIS_STYLE = [
...
"Shared font styling used by both `_mga` figures (one option per line, unindented)."
const MGA_FONT_STYLE = [
...
"""
function plot_ratio_dots_mga(df, names, budgets, basepath)
    a, b = names
    a_tex, b_tex = figure_algorithm_name(a), figure_algorithm_name(b)
...
"""
function plot_ratio_hist_mga(df, names, budgets, basepath)
    a, b = names
    a_tex, b_tex = figure_algorithm_name(a), figure_algorithm_name(b)
    all_ratios = 100 .* collect(skipmissing(df.ratio))
    isempty(all_ratios) && return
...
function analyse(num)
    ...
    if num in (3, 4)
        ...
        plot_ratio_dots_mga(df, spec.names, spec.budgets, joinpath(ROOT, "figs", "exp$(num)-ratiodots_mga"))
        plot_ratio_hist_mga(df, spec.names, spec.budgets, joinpath(ROOT, "figs", "exp$(num)-ratiohist_mga"))
    end
end
